      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-bold text-ink mb-4">
            Médicos que pararam de depender da sorte
          </h2>
          <p className="text-base sm:text-lg text-slate-500 max-w-xl mx-auto">
            Veja o que dizem as clínicas que trocaram o marketing no escuro pela
            previsibilidade da Runner.
          </p>

          <a
            href={siteConfig.linkGoogleBusiness}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2.5 mt-6 px-5 py-2.5 rounded-full bg-white border border-slate-200 shadow-sm hover:shadow-md transition-shadow"
          >
            <GoogleG className="w-5 h-5" />
            <span className="text-sm font-semibold text-slate-700">
              {siteConfig.notaGoogle}
            </span>
            ...
            <span className="text-sm text-slate-400">
              · {siteConfig.qtdAvaliacoes} avaliações
            </span>
          </a>
        </div>
      </div>

      {/* Carrossel automático (marquee). Pausa ao passar o mouse.
          Dois grupos idênticos lado a lado: a animação desloca exatamente
          a largura de um grupo (-50%), garantindo loop contínuo sem emenda. */}
      <div className="marquee-pause relative w-full overflow-hidden">
        <div className="marquee-track flex w-max">
          <div className="flex gap-4 pr-4 shrink-0">
            {avaliacoes.map((av, i) => (
              <CardAvaliacao key={`a-${i}`} av={av} />
            ))}
          </div>
          <div className="flex gap-4 pr-4 shrink-0" aria-hidden="true">
            {avaliacoes.map((av, i) => (
              <CardAvaliacao key={`b-${i}`} av={av} />
            ))}
          </div>
        </div>
      </div>
